...
class SequenceValidator:
    def __init__(self, file_list, manifest):
        ...
        try:
            with open(self.manifest, 'r') as register:
                ...
                if self.line_count != len(self.file_list):
                    logger.critical(f"Manifest line count mismatch: line count: {self.line_count} != file count: {len(self.file_list)}")
                else:
                    logger.info(f"Manifest line count match: line count: {self.line_count} == file count: {len(self.file_list)}")
    ...
    def count_file_sequence(self):
        try:
            ...
            for count in self.file_list:
                file_count = os.path.basename(count).split("_")[5].split(".")[0]
                
                while sequence_count < int(file_count):
                    self.missing_sequence.append(sequence_count)
                    logger.critical(f"Missing sequence: {sequence_count}")
                    sequence_count += 1
